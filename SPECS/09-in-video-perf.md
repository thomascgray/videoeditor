...
Spec 08 designed a VideoDecoder (WebCodecs) + Web Worker export pipeline. Per `TASKS/08-webcodecs-refactor.md`, it was fully built (`src/lib/videoDecoder.ts`, `src/lib/exportWorker.ts`, `src/lib/exportWorkerTypes.ts`), hit codec-rejection and deadlock bugs, was rewritten to fix them (two-phase demux/decode architecture), and then on 2026-03-29 was **dropped anyway** in a "simplification" step: export went back to main-thread HTMLVideoElement seeking. The task log explicitly records the consequence:

> "HTMLVideoElement seeking is now the clear bottleneck (~27s of the export is seek time)."

The decoder/worker files still exist but are **dead code** — nothing calls `new Worker(...)` or `createVideoFrameSource()` outside `exportWorker.ts` (itself unreferenced). The `exportWithWorker()` glue that once wired them up was deleted from `ffmpegExport.ts` but exists in git history.
...
- **B4. Export off the main thread**: wire up the existing `exportWorker.ts` via a restored `exportWithWorker()` (recoverable from git history — commit before the 2026-03-29 "Simplification"). Audio remains pre-rendered on the main thread (OfflineAudioContext unavailable in workers) and transferred as `RenderedAudio` (already implemented in `exportWorkerTypes.ts` / `exportWorker.ts`).
...
1. **Export approach confirmation**: this spec recommends resurrecting the decoder+worker pipeline (B1–B4) over merely patching the seek race (always-await-`seeked` would fix "framey" but keep exports slow, ~real-time, and main-thread). The pipeline was abandoned once for bugginess — are we happy to re-commit to it given the bugs are now specifically identified (B2/B3) rather than mysterious? *(Recommended: yes.)*
...
4. **Export: worker wiring** (B4) — restore `exportWithWorker()` from git history (commit preceding "Simplification: Drop VideoDecoder + Web Worker", 2026-03-29); `exportWorker.ts` needs only the updated `VideoFrameSource` API + per-object keying; `vite.config.ts` needs `worker: { format: 'es' }` re-added if it was removed.
