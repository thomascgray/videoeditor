...
export default function ImportModal({
  ...
  const handleImport = useCallback(() => {
    ...
    onImport(newObjects);
    onClose();
  }, [previews, onImport, onClose, insertAtTime]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 className="text-lg font-bold text-white">Add Image</h2>
          <button
            onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {previews.map((preview, i) => (
                <div
                  key={i}
                  className="relative group rounded overflow-hidden bg-gray-900"
                >
                  <img
                    src={preview.src}
                    alt={preview.name}
                  ...
                  <p className="text-[10px] text-gray-500 truncate px-1 py-0.5">
                    {preview.name}
                  </p>
                </div>
              ))}
